...
class TaskItemWidget(QWidget):
    ...
    def update_appearance(self):
        font = self.desc_label.font(); font.setStrikeOut(self.is_done); self.desc_label.setFont(font) # when task is done, strike out the text
        palette = self.desc_label.palette()
        palette.setColor(self.desc_label.foregroundRole(), Qt.gray if self.is_done else Qt.black)
        self.desc_label.setPalette(palette)
        self.datetime_label.setVisible(not self.is_done)